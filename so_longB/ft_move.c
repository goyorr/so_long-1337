...
int	moveup(t_data *data, int move)
{
	...
	&& data->map[(data->playery / 64) - 1][data->playerx / 64] != 'E')
	{
		...
		else if (data->map[(data->playery / 64) - 1][data->playerx / 64] == 'N')
			exit(write(1, "you lost!", 9));
		...
		mlx_string_put(data->mlx, data->win, 0, 0, 0x00FF00, "MOVES:");
		mlx_string_put(data->mlx, data->win, 0, 20, 0x00FF00, ft_itoa(move));
	}
...
int	moveleft(t_data *data, int move)
{
	...
	&& data->map[data->playery / 64][(data->playerx / 64) - 1] != 'E')
	{
		...
		else if (data->map[data->playery / 64][(data->playerx / 64) - 1] == 'N')
			exit(write(1, "you lost!", 9));
		mlx_put_image_to_window(data->mlx, data->win, data->img.fr,
			data->playerx, data->playery);
		data->playerx -= 64;
